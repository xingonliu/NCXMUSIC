...
- 状态：Proposed；直连 HTTPS 方案已实现，状态机、队列算法与 IPC 契约已验证，真实音频格式与 Range 行为待验证
- 日期：2026-08-06
- 对应 Spike：T-03
...
真实音频格式（MP3/AAC/FLAC）的加载、`Range`/206/416、过期 URL 的真实表现，以及 Windows/macOS 后台、最小化、锁屏与睡眠恢复行为均未验证。这些需要真实登录账号或本地支持 Range 的音频服务，不能用假数据替代。T-03 因此暂不标记 `pass`。
