...
候选实现为独立 `InputHookHost` 加 `uiohook-napi`。它只接收当前配置组合键并向 Main 上报 `pressed | released`，其他键盘/鼠标事件不跨进程、不写日志。Hook 只在语音快捷键启用时运行，停止功能或退出应用时必须卸载。
...
任一平台不能稳定满足条件时，该平台禁用全局按住说话并解释原因，只保留小云输入区按住麦克风按钮；不能把行为静默改成“按一次开始、再按一次结束”。
