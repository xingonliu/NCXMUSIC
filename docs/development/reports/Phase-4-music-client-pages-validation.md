...
- 沉浸歌词按已唱、正在唱和未唱三态建立透明度、缩放与模糊层级；`yrc` 逐字填充严格使用上游 `startMs` 与 `durationMs` 的线性进度，不再强加最短时长或二次缓动。播放期间由 `requestAnimationFrame` 的单调页面时钟连续推进，只在暂停、seek、后台恢复或权威位置向前时重设锚点，不再把每一帧回拉到离散 `timeupdate` 采样。字内视觉使用固定未唱底字与白色伪元素覆盖层的 `clip-path` 裁切，不再切换透明渐变文字或逐帧改变模糊光晕；字词在起音帧立即进入 active，并以 600ms 的 `cubic-bezier(0.16, 1, 0.3, 1)` 一次性过渡上抬 1.5px、缩放至 1.02 倍及呈现柔和阴影。上抬标记跨 active → past 保持不变，且不叠加参考 demo 的长音循环颤动，避免再次引入抖动。歌词焦点跨行时才更新 Vue 响应式时间轴，过期动画循环通过代次失效。合唱左右声部只保留歌词正文，不展示男/女、和声、合唱或括号内演唱者标签；长句与超长逐字音节按面板可用宽度换行，激活态缩放后仍不越界。
- PlayerBar 与沉浸页共用固定样式音乐进度条：轨道、白色填充及 Hover 光晕沿用 NcxMusicWeb 既有实现，不跟随封面或主题背景换色；拖动期间只更新本地预览，按匹配的 Pointer ID 释放后才向播放器提交一次 seek，`pointercancel` 与窗口失焦会放弃预览。切歌以 `trackId` 重建进度条并清空尚未结束的 Pointer 状态；时长未知时不把当前位置或 `1ms` 冒充总时长，播放器也忽略同位置 no-op seek，避免进入无法收到 `seeked` 的锁死状态。
- 当前歌词或长间奏节点通过质量、刚度和阻尼积分弹簧滚动至容器 38% 高度；鼠标滚轮或触摸浏览会立即暂停跟随，连续交互重置 4 秒闲置计时，恢复后弹簧回到当前焦点。点击任意歌词行会按 `lineStartMs` 跳播并立即恢复跟随。
- 歌词时间轴中的真实空白达到 4 秒时插入三点式间奏节点；空白起点取此前所有重叠声部的最晚结束时间，短副唱或对唱提前结束时不会打断仍在演唱的长句。声部标签与整行括号文本归一化为副唱并右侧缩进。当前标准歌词与歌曲实体没有 BPM 字段，因此间奏点使用中性呼吸节律，不宣称与真实 Tempo 对齐。
