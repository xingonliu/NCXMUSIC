...
- PlayerBar 封面与沉浸页封面通过 View Transition 共享元素连续缩放，展示层从窗口底部展开；减少动态效果时直接切换。
- 沉浸页顶部短杆支持鼠标和触控下拉：短杆严格跟手移动，封面根据沉浸大封面与 PlayerBar 小封面的实时矩形连续插值位置、尺寸和圆角，标题、控制器、歌词和背景只渐隐而不整体下坠；达到距离阈值或快速下甩时，从当前插值状态接续共享元素过渡，未达到阈值则沿原路径回弹。短杆保持单根加粗 SVG 横线，悬停仅显示泛光，不改变形状或增加底色。
- 沉浸歌词自动跟随当前播放行，用户主动滚动时暂停跟随，点击歌词可跳转到对应播放时间。
