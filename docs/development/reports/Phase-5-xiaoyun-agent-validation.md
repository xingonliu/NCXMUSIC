...
Entity Resolver 只从本轮标准实体事实池解析 `current`、稳定 ID 和语义名称；两个同名精确候选必须返回消歧，不擅自选第一项。SelectionCard 的实体展示字段由 Runtime 从事实池生成，结果只返回 `selectedOptionKeys` 与 `selectedRefs`，不保存待执行回调。
...
音乐 M1～M4 与命令 S1～S4 均为纯函数策略；等级只能放行已经注册且通过 Schema 的能力，不能创建新 Tool、Action 或 Capability。M1 对所有音乐副作用审批，M2 放行播放器/队列，M3 放行音乐库/歌单/公开社交，M4 才放行高影响账户动作。
...
- ToolExecutionCard 使用 `@lucide/vue` 显式语义映射：搜索、播放、队列/歌单、评论、账户、选择、能力网关和通用音乐均使用对应 Lucide 图标；左侧图标、短状态、耗时和右侧状态点遵循用户提供的工具列表示例，展开后只显示脱敏参数与结果。
...
- `tests/unit/agent-tool-registry.test.ts`：10+2 正向注册、通用签到风险分类和未知 Action 拒绝；
...
- `tests/unit/provider-profile-store.test.ts`：公开快照和磁盘均无凭据明文；
- `tests/integration/agent-main-loop.test.ts`：搜播真实回执与 M1 拒绝零执行；
- `tests/e2e/phase5-agent-workflow.spec.ts`：搜播、歌单写入、SelectionCard、拒绝零执行和播放器状态桥；
- `tests/unit/phase5-agent-ui.test.ts`：正式路由、Lucide 映射、状态点和卡片交互约束。
