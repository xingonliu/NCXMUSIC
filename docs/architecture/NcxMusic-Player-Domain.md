...
- “关闭窗口时”设置只有 `minimize` 和 `quit` 两个值，默认 `minimize`，不增加 `ask` 或其他第三状态。
- `minimize`：拦截关闭按钮并最小化主窗口，保留 Renderer、AudioHost 和 Utility Process，当前音乐继续播放。
...
- 关闭策略为 `minimize` 时窗口最小化且播放不中断；为 `quit` 或明确退出时完成快照后停止媒体并释放所有资源。
