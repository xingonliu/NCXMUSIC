...
> 访谈状态：2026-08-04 重新开启播放架构讨论；目前确认 Vue SPA、根层唯一 AudioHost、按路由显示/隐藏 PlayerBar，以及单曲插入并立即播放与集合上下文替换队列规则，其余技术结构和行为规则仍是讨论底稿。
...
1. URL 可能过期：重新解析并重试一次，禁止对写操作式接口盲目重试。
2. 网络暂时失败：展示重试状态；自动重试次数待确认。
3. VIP、地区限制、下架和无版权：给出明确不可播放原因。
4. 是否在不可播放后自动跳到下一首、连续失败多少首后停止，属于待确认产品决策。
...
持久化只有两种自洽方案，不能同时写“保存位置”和“进度不持久化”：

### 方案 A：仅保存偏好

- 保存 `volume`、`muted`、`mode`。
- 不保存队列、当前歌曲和位置；启动后保持 idle。

### 方案 B：恢复上次现场

- 保存稳定歌曲/队列引用、规范队列快照、当前 `queueItemId`、`positionMs`、mode 和写入时间，不保存临时 URL。
- 启动后重新解析当前歌曲 URL，以 `autoplay=false` 装载；元数据就绪后 seek，保持 ready/paused，禁止自动出声。
- 来源已失效时使用保存的队列快照恢复；歌曲不可用时展示原因，不静默选择另一首。
- 快照需要 Schema 版本、账户 ID、过期策略和原子写入。
