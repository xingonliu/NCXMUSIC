...
Phase 6 的 `MemoryWriter` 已落地为账户隔离的 `ConversationMemoryService`：最近用户消息空闲 10 分钟后把尚未归档的稳定消息写入会话块，生成确定性摘要、关键词与重要性，并由外部内容 FTS5 Trigger 同步索引。新 Turn 只按当前目标检索最多 5 个高价值块，组合 FTS 相关性、重要性与新鲜度写入 Working Memory；中文连续文本使用参数化二元词有限召回，不把原始 FTS 运算符拼入 SQL。SQLite 是权威来源，`working-memory.json` 只作原子快照并可从库内重建。

音乐人格画像由独立后台 Job 执行，不占用普通聊天 Turn。初始化、更新、重新生成与失败重试必须来自 Renderer 用户命令；启动时只做喜欢集合与歌单摘要的轻量变化检查，不静默采集歌单详情或调用模型。画像子循环只暴露内部只读 `get_profile_evidence_page`，单页参数经 Schema 校验且最多调用 6 次；该 Tool 不注册到主会话 10+2 Tool Registry。成功结果与变化基线原子替换，失败保留旧画像及已聚合数据；用户纠正、隐藏和补充在 ContextBuilder 中始终高于模型结论。
