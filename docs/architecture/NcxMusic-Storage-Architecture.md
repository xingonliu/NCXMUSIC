...
│  │     ├─ account.sqlite        # 对话、块、摘要、索引、审计与结构化画像
...
- 对话、消息块、摘要和画像结构化数据写入 SQLite；全文或向量检索策略仍可通过后续迁移增加，不在存储层提前绑定。
...
- 自动保留期限尚未确认；在确认前不后台自动删除用户聊天、画像或 Working Memory。
- 日志和临时缓存使用独立、可配置的短期滚动策略，不能依赖用户手动清理。
