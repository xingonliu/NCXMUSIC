...
2. 中部放队列、收藏、歌单和小云相关动作。
3. 导航与复制链接放在后部。
4. 删除、移除、取消收藏等破坏性动作放在最后一组。可恢复的取消收藏/移除直接执行并显示 Toast；删除自建歌单等不可逆操作才使用 AlertDialog。
5. 单层菜单尽量不超过 10 项；歌单选择、多个歌手和小云能力使用 Submenu。
...
- 取消收藏：末组 Danger，直接执行并显示 Toast，不弹 AlertDialog。
...
1. 首版采用本文件列出的歌曲、队列、歌单、专辑、歌手、侧栏和 Agent 菜单内容。
2. “交给小云”进入歌曲、歌单、专辑和歌手右键菜单。
3. 取消歌曲/歌单/专辑/歌手收藏以及从歌单或队列移除时直接执行并显示 Toast，不弹 AlertDialog。
4. 删除自建歌单等不可逆操作使用 AlertDialog；审批和 Agent 权限卡片仍使用各自专用组件。
