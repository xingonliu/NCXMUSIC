...
业务组件不能使用任意 Z-Index。ApprovalCard 与 SelectionCard 以绝对定位浮层完整覆盖 AgentComposer 输入框，不参与文档流、不推动 Agent 会话，也不提升到 `layer-modal`；只有跨页面的未决审批恢复层才使用 `layer-modal`。
...
| Variant | 资源尺寸 | UI 场景 |
| --- | ---: | --- |
| `backdrop` | 40 × 40 | 沉浸歌词动态背景专用输入，不用于可辨识封面展示 |
...
- 模糊背景不得请求无尺寸原图；沉浸歌词应单独请求 `backdrop`，避免把 `hero` 资源上传到仅需要色彩分布的 GPU 管线。CSS 降级层允许复用同一 `backdrop` URL。
- UI Lab 必须同时展示五档可辨识封面资源以及背景专用输入的网络状态，并覆盖亮暗主题、加载/失败、低网速、长列表和高 DPI 状态；网络面板应确认请求尺寸与 Variant 一致。

#### 4.3.2 沉浸歌词流体背景契约

沉浸歌词页使用 `FluidMeshBackground` 请求当前封面的 40 × 40 `backdrop` 版本，并采用 Apple Music 网页歌词场景同形的 PixiJS 8 WebGL 后处理管线；前景 `MediaArtwork` 仍使用高清资源。背景不得先提取主色或生成噪声网格，而是把同一张低分辨率封面实例化为四张 Sprite，以视口宽度的 125%、80%、50%、25% 依次叠放。两张大图原地差速旋转，两张小图反向自转并沿差速圆形轨道移动；第四层纵向偏心以视口宽度而不是高度为基准。

- Pixi 滤镜链固定按「四层封面 Sprite → `TwistFilter` → 五个 `KawaseBlurFilter` → `AdjustmentFilter`」执行。Twist 参数为 `angle = -3.25`、`radius = 900`、中心跟随视口；五级 Kawase 的 `(strength, quality)` 依次为 `(5, 1)`、`(10, 1)`、`(20, 2)`、`(40, 2)`、`(80, 2)`；最终色彩参数为 `saturation = 2.75`、`brightness = 0.7`、`contrast = 1.9`。不要以 Gaussian Blur、自写九点核、Sprite 羽化或抖动代替这些已核验参数。
- Pixi 应用使用现有 Canvas、CSS 视口尺寸、`resolution = 1`、WebGL 偏好、`powerPreference: low-power` 和 15 fps 上限。连续流动相位由时间驱动，另通过 2048 点 FFT 只读取 50～120 Hz Bass / Kick 能量。低频采用快速起音、慢速释放后，最多叠加 10% 封面缩放、0.2 rad 相位偏移和 4% Twist 强度，不能让人声或高频造成画面闪烁。
- 切歌使用三个各含四张 Sprite 的封面组，从当前可见权重开始，以约 1.67 秒线性曲线交叉淡化到新封面。连续快速切歌时重定向当前贡献最小的槽、重新归一化其余可见贡献，完成后释放不可见纹理；不得闪回已经离场的封面或切到 CSS 降级层。
- 暂停播放后，低频律动先释放到 0，四层封面的时间推进倍率在 1.2 秒内平滑降到 0 并停在当前相位；恢复播放时从同一相位平滑回到完整速度，不得重置旋转角或轨道位置。
- 背景上方依次覆盖 50% 黑色与 5% 白色的平面合成层，不使用纵向渐变或边缘加深的径向暗角；白色歌词不能依赖文字描边弥补背景过亮。
- 封面加载必须支持切歌取消和跨域纹理校验；切歌瞬间首次请求失败时执行一次可取消的短延迟重试。现有 Pixi 实例切换纹理失败时，必须在当前页面内换用全新 Canvas，并用当前已解码封面重建管线；禁止在同一 Canvas 销毁旧 Pixi 后立即重建，因为 Pixi 会强制丢失旧 WebGL 上下文，其延迟事件可能覆盖新实例。浏览器自然恢复 WebGL 上下文时复用现有 Pixi 实例，不能再次销毁刚恢复的上下文。WebGL 初始化、连续纹理加载失败或上下文丢失时保留同一封面的 CSS 高饱和弥散层作为降级。
- 页面不可见时暂停 Pixi Ticker；`prefers-reduced-motion` 下冻结四层封面变换和音频脉冲，但切歌保留约 180 ms 的短透明度淡化，避免亮度骤变。
...
ApprovalCard 与 SelectionCard 只在待处理期间以紧凑、与输入框等宽的浅灰磨砂卡片完整覆盖输入框，浅色与暗色模式均使用 30% 不透明度的中性灰背景，并通过 32px 背景高斯模糊与卡片自身的 `box-shadow` 建立层级；暗色模式使用更深的中性灰，卡片背景不得使用渐变或额外饱和滤镜，浮层祖先不得使用 `filter`，避免创建 Backdrop Root 隔离下层内容。不使用卡片或选项描边，也不为浮层预留会话空间；对话时间线用灰底轻量消息记录“等待批准/等待选择”，完成后原位更新为“已批准/已拒绝/已选择”或对应的取消、过期状态。状态消息只用于记录，不提供任何重复提交入口。
...
| AlertDialog | 不可逆或高风险的人类主动操作确认 | 小云的权限审批、普通保存 |
...
| ApprovalCard | 小云工具执行前的批准/拒绝 | 用户主动操作的普通二次确认 |
| SelectionCard | 小云需要用户在明确候选中选择一个对象 | 审批、风险确认、选择后直接执行副作用 |
