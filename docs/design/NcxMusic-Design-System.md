...
沉浸歌词页使用 `FluidMeshBackground` 复用当前 `hero` 封面，并采用 Apple Music 网页歌词场景同形的实时纹理后处理管线。背景不得先提取主色或生成噪声网格，而是把完整封面作为 WebGL 纹理复制四份，以视口宽度的 125%、80%、50%、25% 依次叠放；两张大图原地反向旋转，两张小图同时沿圆形轨道移动。不能为了背景重新请求无尺寸原图。

- WebGL 管线固定按「四层封面 Sprite → 平方衰减 Twist → 多次 Kawase Blur → 2.75 倍 Saturation」执行；Twist 最大角为 -3.25 rad，作用半径随视口宽度缩放。较小 Sprite 的四边必须以 8% 内边距羽化叠加，模糊使用含中心和轴向采样的九点帐篷核，最终输出加入低于 1/255 的抖动；不得让方形硬边经过稀疏对角采样后形成固定竖纹。
- 内部画布最大为 640 × 420、上限 15 fps，并使用 `powerPreference: low-power`；连续流动相位由时间驱动，另通过 2048 点 FFT 只读取 50～120 Hz Bass / Kick 能量。低频采用快速起音、慢速释放后，最多叠加 10% 封面缩放、0.2 rad 旋转偏移和 4% Twist 强度，不能让人声或高频造成画面闪烁。
- 切歌在 GPU 内通过三个封面纹理槽从当前可见权重开始，用 1.5 秒平滑曲线交叉混合到新封面；连续快速切歌必须重定向当前混合权重，不能闪回已经离场的封面或切到 CSS 降级层。
- 暂停播放后，低频律动先释放到 0，四层封面的时间推进倍率在 1.2 秒内平滑降到 0 并停在当前相位；恢复播放时从同一相位平滑回到完整速度，不得重置旋转角或轨道位置。
- 背景上方保留纵向对比遮罩，不使用边缘加深的径向暗角，白色歌词不能依赖文字描边弥补背景过亮。
- 封面加载必须支持切歌取消和跨域纹理校验；WebGL 初始化、纹理加载失败或上下文丢失时保留同一封面的 CSS 高饱和弥散层作为降级。
- 页面不可见时暂停动画帧；`prefers-reduced-motion` 下冻结四层封面变换并保留当前静态色场。
