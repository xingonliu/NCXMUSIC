...
ApprovalCard 与 SelectionCard 只在待处理期间以紧凑、与输入框等宽的浅灰磨砂卡片完整覆盖输入框，使用可透出下层内容的 32px 背景高斯模糊与卡片自身的 `box-shadow` 建立层级；浮层祖先不得使用 `filter`，避免创建 Backdrop Root 隔离下层内容。不使用卡片或选项描边，也不为浮层预留会话空间；对话时间线用灰底轻量消息记录“等待批准/等待选择”，完成后原位更新为“已批准/已拒绝/已选择”或对应的取消、过期状态。状态消息只用于记录，不提供任何重复提交入口。
