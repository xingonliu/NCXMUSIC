...
沉浸歌词页使用 `FluidMeshBackground`，不能再以整张封面的缩放动画代替动态网格。组件复用当前 `hero` 封面，在 72 × 72 离屏画布上建立五位 RGB 直方图并执行加权 K-Means；四个输出节点分别承担主色、辅色、高光和暗部职责，并在 OKLCH 中把最终明度限制为 15%～45%。跨域策略不允许读取封面像素时，使用封面 URL 生成稳定的深色调色板，同时保留同一资源的 CSS 弥散层作为降级，不能为背景重新请求无尺寸原图。

- WebGL 使用低分辨率全屏四边形，内部画布最大为 420 × 320、上限 30 fps；CSS 全屏放大后统一应用 80 px 高斯弥散。
- Fragment Shader 使用四个颜色控制节点、低频 2D Simplex Noise 空间扭曲和 3.5% 动态 Grain；不可在 DOM 中叠加大量渐变节点模拟流体。
- 切歌必须从当前画面颜色开始，在 1.5 秒内按四组 RGB 向量线性插值到新调色板；连续快速切歌不能回跳到更早的起始色。
- 背景上方必须保留中央微暗、边缘加深的径向暗角和纵向对比遮罩，白色歌词不能依赖文字描边弥补背景过亮。
- 页面不可见时暂停动画帧；暂停播放时降低流速；`prefers-reduced-motion` 下停止空间位移但仍允许颜色平滑切换。WebGL 初始化失败或上下文丢失时必须保持可读的封面弥散降级层。
