...
`SelectionCard` 是 `request_user_selection` Tool 的内联快捷提问组件，帮助用户在 2~5 个结构化答案中点选一项、减少文字输入。它允许在同一卡片混排两种 Option Row：音乐实体使用 `MediaArtwork thumbnail` 并展示名称、歌手、专辑和权益小标；普通文字选项展示标题和一行可选说明。整行选项是单一可点击答案，支持键盘焦点、方向键移动和 Enter 选择。卡片提供文案固定为“取消”的次要按钮，不使用“批准/拒绝”文案；点击只提交回答，绝不执行选项文案描述的动作。固定 10 分钟到期后进入只读过期态；选择、取消和过期状态都必须保留结果并锁定全部交互。
...
| SelectionCard | 小 N 需要用户在明确候选中选择一个对象 | 审批、风险确认、选择后直接执行副作用 |
