...
12. 接口被测试不代表自动成为 NcxMusic 页面能力或小 N 能力。生产注册仍须经过 Capability Catalog、安全分级与产品范围审查。
...
inferred、unknown 和 conflicting 字段不得直接进入标准领域模型、UI 文案、用户画像或小 N 系统提示词。可以保留在原始扩展字段中，等待后续解析。
