...
function parseMusicProfileAnalysis(value: string): MusicProfileAnalysis {
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new Error(`模型画像格式校验未通过：${error.issues.map((issue) => issue.message).join('；')}`)
    }
    if (error instanceof SyntaxError) {
      throw new Error('模型返回的内容无法解析为有效 JSON。')
    }
    throw error instanceof Error ? error : new Error('模型没有返回有效画像 JSON。')
  }
}
