/** Spring, press tracking and button squash copied from D:/code/chat/docs/liquid-glass/app.js. */

// -- Type Definitions

export interface GlassDeform {
  tx: number
  ty: number
  sx: number
  sy: number
  progress: number
  ox: number
  oy: number
}

// -- Functions

export function tanh(x: number): number {
  const e = Math.exp(2 * Math.max(-20, Math.min(20, x)))
  return (e - 1) / (e + 1)
}

export function lerp(a: number, b: number, t: number): number {
  return a + (b - a) * t
}

export function clamp(v: number, a: number, b: number): number {
  return Math.max(a, Math.min(b, v))
}

/** androidx.compose.animation.core.SpringSimulation closed-form step. */
export class Spring {
  value: number
  target: number
  vel = 0
  z: number
  w: number
  threshold: number

  constructor(value: number, dampingRatio: number, stiffness: number, threshold = 0.001) {
    this.value = value
    this.target = value
    this.z = dampingRatio
    this.w = Math.sqrt(stiffness)
    this.threshold = threshold
  }

  set(t: number): void {
    this.target = t
  }

  snap(t: number): void {
    this.value = this.target = t
    this.vel = 0
  }

  get settled(): boolean {
    return Math.abs(this.value - this.target) < this.threshold && Math.abs(this.vel) < this.threshold * 25
  }

  step(dt: number): number {
    if (dt <= 0) return this.value
    if (this.settled) {
      this.value = this.target
      this.vel = 0
      return this.value
    }
    const z = this.z
    const w = this.w
    const disp = this.value - this.target
    const v0 = this.vel
    let x: number
    let v: number
    if (z > 1) {
      const adj = Math.sqrt(z * z - 1)
      const gp = w * (z + adj)
      const gm = w * (z - adj)
      const b = (gm * disp - v0) / (gm - gp)
      const a = disp - b
      const e1 = Math.exp(-gm * dt)
      const e2 = Math.exp(-gp * dt)
      x = a * e1 + b * e2
      v = a * -gm * e1 + b * -gp * e2
    } else if (Math.abs(z - 1) < 1e-5) {
      const a = disp
      const b = v0 + w * disp
      const e = Math.exp(-w * dt)
      x = (a + b * dt) * e
      v = (a + b * dt) * e * -w + b * e
    } else {
      const wd = w * Math.sqrt(1 - z * z)
      const a = disp
      const b = (1 / wd) * (z * w * disp + v0)
      const e = Math.exp(-z * w * dt)
      const s = Math.sin(wd * dt)
      const c = Math.cos(wd * dt)
      x = e * (a * c + b * s)
      v = x * -z * w + e * (-wd * a * s + wd * b * c)
    }
    this.value = x + this.target
    this.vel = v
    return this.value
  }
}

/** Kyant InteractiveHighlight — press spring 0.5/300, drag snap, release spring-back. */
export class InteractiveHighlight {
  press = new Spring(0, 0.5, 300, 0.001)
  px = new Spring(0, 0.5, 300, 0.5)
  py = new Spring(0, 0.5, 300, 0.5)
  startX = 0
  startY = 0
  down = false
  moved = false

  get offsetX(): number {
    return this.px.value - this.startX
  }

  get offsetY(): number {
    return this.py.value - this.startY
  }

  start(lx: number, ly: number): void {
    this.down = true
    this.moved = false
    this.startX = lx
    this.startY = ly
    this.px.snap(lx)
    this.py.snap(ly)
    this.press.set(1)
  }

  move(lx: number, ly: number): void {
    if (!this.down) return
    if (Math.abs(lx - this.startX) + Math.abs(ly - this.startY) > 10) this.moved = true
    this.px.snap(lx)
    this.py.snap(ly)
  }

  end(): void {
    this.down = false
    this.press.set(0)
    this.px.set(this.startX)
    this.py.set(this.startY)
  }

  step(dt: number): void {
    this.press.step(dt)
    this.px.step(dt)
    this.py.step(dt)
  }

  get active(): boolean {
    return this.down || !this.press.settled || !this.px.settled || !this.py.settled
  }
}

export function buttonDeform(w: number, h: number, hl: InteractiveHighlight): GlassDeform {
  const progress = hl.press.value
  const scale = lerp(1, 1 + 4 / h, progress)
  const maxOffset = Math.min(w, h)
  const ox = hl.offsetX
  const oy = hl.offsetY
  const tx = maxOffset * tanh(0.05 * ox / Math.max(maxOffset, 1))
  const ty = maxOffset * tanh(0.05 * oy / Math.max(maxOffset, 1))
  const maxDragScale = 4 / h
  const ang = Math.atan2(oy, ox)
  const maxDim = Math.max(w, h)
  const sx = scale + maxDragScale * Math.abs(Math.cos(ang) * ox / maxDim) * Math.min(w / h, 1)
  const sy = scale + maxDragScale * Math.abs(Math.sin(ang) * oy / maxDim) * Math.min(h / w, 1)
  return { tx, ty, sx, sy, progress, ox, oy }
}
