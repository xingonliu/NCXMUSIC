...
export class CredentialLeaseCoordinator {
  ...
  currentLease(): ActiveLeaseMetadata | undefined {
    return this.activeLease && this.activeLease.expiresAt > this.now()
      ? { ...this.activeLease }
      : undefined
  }
  ...
  private request(command: CredentialControlCommand, timeoutMs: number): Promise<CredentialControlEvent> {
