...
export function isNeteaseAuthCookie(cookie: Pick<Cookie, 'domain'>): boolean {
  ...
  return domain === '163.com' || domain === 'music.163.com' || domain.endsWith('.music.163.com')
}
...
function structurallyValidMusicU(value: string): boolean {
...
export class CookieSessionRepository {
  ...
  async inspect(): Promise<CredentialInspection> {
    ...
    const credential = cookies.find((cookie) => cookie.name === 'MUSIC_U')
    if (!credential) {
      return { kind: 'guest', cookieCount: cookies.length }
    }
