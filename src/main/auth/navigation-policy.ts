...
export function isNeteaseLoginHost(hostname: string): boolean {
...
export function isAllowedLoginNavigation(value: string): boolean {
  try {
    ...
    return url.protocol === 'https:' && isNeteaseLoginHost(url.hostname)
  } catch {
    return false
  }
}
...
export const NETEASE_LOGIN_URL = 'https://music.163.com/#/login'
