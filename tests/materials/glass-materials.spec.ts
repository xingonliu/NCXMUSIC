...
test('page shell, static artwork controls and player compose with the new layout', async () => {
  await page.goto('http://127.0.0.1:5198/tests/fixtures/glass-materials/pages.html')
  const sidebar = page.locator('.ncx-sidebar')
  await expect(sidebar.locator(':scope > .ncx-glass-surface')).toHaveAttribute('data-material', 'dialog')
  await expect(page.locator('.player-bar-glass > .ncx-glass-surface')).toHaveAttribute('data-material', 'blur')
  await page.locator('.ncx-cover-img').evaluate(image => { (image as HTMLImageElement).loading = 'eager' })
  await expect.poll(() => page.locator('.ncx-cover-img').evaluate(image => (image as HTMLImageElement).naturalWidth)).toBeGreaterThan(0)
  await expect(page.locator('.ncx-cover-play-btn .ncx-glass-surface')).toHaveAttribute('data-material', 'clear')
  await expect(page.getByTestId('hero-play').locator('.ncx-glass-surface')).toHaveAttribute('data-material', 'tinted')
  await expect(page.getByTestId('hero-more').locator('.ncx-glass-surface')).toHaveAttribute('data-material', 'clear')
  const position = await sidebar.boundingBox()
  await page.screenshot({ path: 'output/playwright/materials/page-light.png' })
  await page.locator('.ncx-content-area').evaluate(el => { el.scrollTop = 280; el.dispatchEvent(new Event('scroll')) })
  await expect.poll(() => page.locator('.ncx-page-header').evaluate(el => el.style.getPropertyValue('--ncx-header-opacity'))).toBe('1')
  expect(await sidebar.boundingBox()).toEqual(position)
  await page.screenshot({ path: 'output/playwright/materials/page-scrolled.png' })
  await page.getByTestId('settings-apply').click()
  await expect(sidebar.locator(':scope > .ncx-glass-surface')).toHaveAttribute('data-material', 'blur')
  await page.locator('html').evaluate(el => { el.dataset['theme'] = 'dark' })
  await page.screenshot({ path: 'output/playwright/materials/page-settings-dark.png' })
})
