...
describe('十项音乐体验 UI 契约', () => {
  ...
  it('集合详情提供完整主操作组，歌手页覆盖六个核心内容区', async () => {
    ...
    expect(collection).toContain('<MusicDetailHero :artwork-url="collection.artworkUrl">')
    expect(collection).not.toContain('<Cover')
    ...
    expect(artist).toContain('<MusicDetailHero')
    expect(artist).toContain('size="prominent"')
    expect(contentStyles).toContain('.artist-page .virtual-track-list')
    expect(contentStyles).toContain('overflow: visible')
  })
})
