...
describe('Apple Music 网页端同形背景参数', () => {
  it('严格使用 125%、80%、50%、25% 四层封面和 15 fps 上限', () => {
    ...
    expect(APPLE_MUSIC_WEB_BACKGROUND_CONFIG.layerRotationSpeeds)
      .toEqual([0.09, -0.24, -0.18, 0.12])
    expect(APPLE_MUSIC_WEB_BACKGROUND_CONFIG.maximumFps).toBe(15)
    expect(APPLE_MUSIC_WEB_BACKGROUND_CONFIG.artworkTransitionMs).toBe(1_500)
    expect(APPLE_MUSIC_WEB_BACKGROUND_CONFIG.motionTransitionMs).toBe(1_200)
    ...
    expect(APPLE_MUSIC_WEB_BACKGROUND_CONFIG.lowFrequencyScalePulse).toBe(0.1)
    expect(APPLE_MUSIC_WEB_BACKGROUND_CONFIG.spriteEdgeFeather).toBe(0.08)
  })
  ...
  it('两张大封面原地反向旋转，两张小封面沿圆形轨道移动', () => {
    ...
    expect(initial[3]).toMatchObject({ centerX: 800, centerY: 330, rotation: 0 })
