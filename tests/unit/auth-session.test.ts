...
describe('auth session state', () => {
  it('invalidates old generations and leases on switch, expiry, and logout', () => {
    ...
    expect(machine.logout()).toMatchObject({ state: 'guest', hasCredentialLease: false })
  })

  it('returns to guest when the login window closes without credentials', () => {
    const machine = new AuthSessionMachine()
    machine.beginLogin()
    expect(machine.closeLoginWindow().state).toBe('guest')
  })
})
...
describe('persistent cookie repository', () => {
  it('recognizes a structurally valid credential and serializes the full isolated set', async () => {
    const cookies = [
  ...
  it('distinguishes guest and invalid cookies and removes the isolated set', async () => {
    const guest = fakeSession([cookie({ name: 'NMTID' })])
    expect(await new CookieSessionRepository(guest.session).inspect()).toMatchObject({ kind: 'guest' })

    const invalid = fakeSession([cookie({ name: 'MUSIC_U', value: 'short' })])
...
describe('credential redaction', () => {
  it('removes cookie headers and named secrets from log text', () => {
    const output = redactSensitiveText(
      'Cookie: alpha=one; MUSIC_U=secret-token\n{"cookie":"alpha=one; beta=two"}'
    )
