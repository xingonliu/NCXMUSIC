...
- 沉浸歌词提供两种可持久选择的体验：新版“动效影院”使用 1:1 高明度舞台、连续三维镜头、穿插当前与前后歌词的三维空间样条、按焦平面 Z 距连续虚化的光学散焦、逐字符反向模糊和真实音频频谱 HUD；经典版保留封面颜色驱动的双栏流体网格播放页。
...
- 设置页提供沉浸歌词版本、歌词翻译、当前歌词位置、动效强度、字号、字重和已唱歌词显示选项；沉浸歌词版本与其他账户偏好一样按账户保存。
...
- 液态玻璃复用 Kyant Backdrop demo 的 WebGL 着色器与按钮/Dialog 光学参数；完整档用于封面上的操作，不完整档用于 Header 与播放栏，面板档用于弹窗和主导航侧栏，见 [液态玻璃材质](docs/development/Liquid-Glass-Materials.md)。
- 全部可见圆角统一使用 60% 连续曲率 Squircle，并收敛到七级尺寸阶梯；实现与使用约束见 [Squircle 形状规范](docs/development/Squircle-Design-System.md)。
- 页面使用全宽滚动画布与覆盖式侧边栏；Header 沿用既有材质并随滚动渐显，音乐详情使用单图主视觉和左下角信息，推荐内容支持横向浏览，见 [页面布局与滚动](docs/development/Page-Layout.md)。
